...
public abstract class WorkDatabase extends RoomDatabase {
        ...
        if (builder.mContext == null) {
            ...
            if (journalMode3 != null) {
                ...
                try {
                    ...
                    StringBuilder outline17 = GeneratedOutlineSupport.outline17("cannot find implementation for ");
                    outline17.append(cls2.getCanonicalName());
                    outline17.append(". ");
                    outline17.append(str);
                    outline17.append(" does not exist");
                    throw new RuntimeException(outline17.toString());
                } catch (IllegalAccessException unused2) {
                    StringBuilder outline172 = GeneratedOutlineSupport.outline17("Cannot access the constructor");
                    outline172.append(cls2.getCanonicalName());
                    throw new RuntimeException(outline172.toString());
                } catch (InstantiationException unused3) {
                    StringBuilder outline173 = GeneratedOutlineSupport.outline17("Failed to create an instance of ");
                    outline173.append(cls2.getCanonicalName());
    ...
    public static String getPruneSQL() {
        StringBuilder outline17 = GeneratedOutlineSupport.outline17("DELETE FROM workspec WHERE state IN (2, 3, 5) AND (period_start_time + minimum_retention_duration) < ");
        outline17.append(System.currentTimeMillis() - PRUNE_THRESHOLD_MILLIS);
        outline17.append(" AND (SELECT COUNT(*)=0 FROM dependency WHERE     prerequisite_id=id AND     work_spec_id NOT IN         (SELECT id FROM workspec WHERE state IN (2, 3, 5)))");
