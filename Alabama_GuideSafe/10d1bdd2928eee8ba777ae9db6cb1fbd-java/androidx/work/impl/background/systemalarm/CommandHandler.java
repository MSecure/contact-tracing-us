    ...
    public void onHandleIntent(Intent intent, int i, SystemAlarmDispatcher systemAlarmDispatcher) {
        ...
        if ("ACTION_CONSTRAINTS_CHANGED".equals(action)) {
            ...
            while (it2.hasNext()) {
                ...
                Logger.get().debug(ConstraintsCommandHandler.TAG, String.format("Creating a delay_met command for workSpec with id (%s)", str2), new Throwable[0]);
                SystemAlarmDispatcher systemAlarmDispatcher2 = constraintsCommandHandler.mDispatcher;
                systemAlarmDispatcher2.mMainHandler.post(new SystemAlarmDispatcher.AddRunnable(systemAlarmDispatcher2, createDelayMetIntent, constraintsCommandHandler.mStartId));
            }
            ...
            if (!z) {
                ...
                try {
                    ...
                    if (workSpec2 == null) {
                        ...
                        if (!workSpec2.hasConstraints()) {
                            ...
                            Alarms.setAlarm(this.mContext, systemAlarmDispatcher.mWorkManager, string, calculateNextRunTime);
                            systemAlarmDispatcher.mMainHandler.post(new SystemAlarmDispatcher.AddRunnable(systemAlarmDispatcher, createConstraintsChangedIntent(this.mContext), i));
                        }
                ...
                synchronized (this.mLock) {
                    ...
                    Logger.get().debug(TAG, String.format("Handing delay met for %s", string2), new Throwable[0]);
                    if (!this.mPendingDelayMet.containsKey(string2)) {
                        ...
                        Logger.get().debug(TAG, String.format("WorkSpec %s is already being handled for ACTION_DELAY_MET", string2), new Throwable[0]);
                    }
                }
