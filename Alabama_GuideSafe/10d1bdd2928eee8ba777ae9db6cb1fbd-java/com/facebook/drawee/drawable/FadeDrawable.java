...
public class FadeDrawable extends ArrayDrawable {
    ...
    public FadeDrawable(Drawable[] drawableArr) {
        super(drawableArr);
        AppCompatDelegateImpl.ConfigurationImplApi17.checkState(drawableArr.length >= 1, "At least one layer required!");
    ...
    public void draw(Canvas canvas) {
        ...
        if (i == 0) {
            ...
            z = updateAlphas(this.mDurationMs == 0 ? 1.0f : BaseViewManager.CAMERA_DISTANCE_NORMALIZATION_MULTIPLIER);
            if (!z) {
                i2 = 1;
            }
            ...
            AppCompatDelegateImpl.ConfigurationImplApi17.checkState(this.mDurationMs > 0);
        ...
        while (true) {
            ...
            int i4 = (this.mAlphas[i3] * this.mAlpha) / 255;
    ...
    public final boolean updateAlphas(float f) {
        ...
        for (int i = 0; i < this.mLayers.length; i++) {
            ...
            iArr[i] = (int) ((((float) (i2 * 255)) * f) + ((float) this.mStartAlphas[i]));
            if (iArr[i] < 0) {
                iArr[i] = 0;
            }
