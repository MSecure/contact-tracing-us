...
public class StackTraceHelper {
    ...
    public static StackFrame[] convertJsStackTrace(ReadableArray readableArray) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (type == ReadableType.Map) {
                ...
                stackFrameArr[i] = new StackFrameImpl(map.getString(PromiseImpl.STACK_FRAME_KEY_FILE), map.getString(PromiseImpl.STACK_FRAME_KEY_METHOD_NAME), (!map.hasKey("lineNumber") || map.isNull("lineNumber")) ? -1 : map.getInt("lineNumber"), (!map.hasKey(COLUMN_KEY) || map.isNull(COLUMN_KEY)) ? -1 : map.getInt(COLUMN_KEY), map.hasKey("collapse") && !map.isNull("collapse") && map.getBoolean("collapse"));
    ...
    public static String formatStackTrace(String str, StackFrame[] stackFrameArr) {
        ...
        for (StackFrame stackFrame : stackFrameArr) {
            outline19.append(stackFrame.getMethod());
            outline19.append("\n");
            outline19.append("    ");
            outline19.append(formatFrameSource(stackFrame));
            outline19.append("\n");
        }
    ...
    public static class StackFrameImpl implements StackFrame {
        ...
        public JSONObject toJSON() {
            ...
            Integer valueOf = Integer.valueOf(getLine());
            Integer valueOf2 = Integer.valueOf(getColumn());
            Boolean valueOf3 = Boolean.valueOf(isCollapsed());
            HashMap hashMap = new HashMap();
            hashMap.put(PromiseImpl.STACK_FRAME_KEY_FILE, file);
            hashMap.put(PromiseImpl.STACK_FRAME_KEY_METHOD_NAME, method);
            hashMap.put("lineNumber", valueOf);
            hashMap.put(StackTraceHelper.COLUMN_KEY, valueOf2);
            hashMap.put("collapse", valueOf3);
    ...
    public static StackFrame[] convertJsStackTrace(JSONArray jSONArray) {
        ...
        for (int i = 0; i < length; i++) {
            try {
                ...
                stackFrameArr[i] = new StackFrameImpl(jSONObject.getString(PromiseImpl.STACK_FRAME_KEY_FILE), jSONObject.getString(PromiseImpl.STACK_FRAME_KEY_METHOD_NAME), (!jSONObject.has("lineNumber") || jSONObject.isNull("lineNumber")) ? -1 : jSONObject.getInt("lineNumber"), (!jSONObject.has(COLUMN_KEY) || jSONObject.isNull(COLUMN_KEY)) ? -1 : jSONObject.getInt(COLUMN_KEY), jSONObject.has("collapse") && !jSONObject.isNull("collapse") && jSONObject.getBoolean("collapse"));
