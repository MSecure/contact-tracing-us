...
public class CameraRollManager extends NativeCameraRollManagerSpec {
    ...
    public static class GetMediaTask extends GuardedAsyncTask<Void, Void> {
        ...
        public void doInBackgroundGuarded(Void[] voidArr) {
            ...
            if (c == 0) {
                ...
                StringBuilder outline17 = GeneratedOutlineSupport.outline17("Invalid filter option: '");
                outline17.append(this.mAssetType);
                outline17.append("'. Expected one of '");
                outline17.append(CameraRollManager.ASSET_TYPE_PHOTOS);
                outline17.append("', '");
                outline17.append(CameraRollManager.ASSET_TYPE_VIDEOS);
                outline17.append("' or '");
                outline17.append(CameraRollManager.ASSET_TYPE_ALL);
                outline17.append("'.");
                promise.reject(CameraRollManager.ERROR_UNABLE_TO_FILTER, outline17.toString());
                return;
            } else {
    ...
    public static boolean putImageInfo(ContentResolver contentResolver, Cursor cursor, WritableMap writableMap, int i, int i2, int i3, int i4, int i5) {
        ...
        outline17.append(cursor.getString(i4));
        Uri parse = Uri.parse(outline17.toString());
        ...
        if (string != null && string.startsWith("video")) {
            try {
                ...
                StringBuilder outline172 = GeneratedOutlineSupport.outline17("Could not get video metadata for ");
                outline172.append(parse.toString());
                FLog.e("ReactNative", outline172.toString(), e2);
                return false;
            }
        }
        if (f <= BaseViewManager.CAMERA_DISTANCE_NORMALIZATION_MULTIPLIER || f2 <= BaseViewManager.CAMERA_DISTANCE_NORMALIZATION_MULTIPLIER) {
            try {
                ...
                StringBuilder outline173 = GeneratedOutlineSupport.outline17("Could not get width/height for ");
                outline173.append(parse.toString());
                FLog.e("ReactNative", outline173.toString(), e3);
                return false;
            }
        }
