...
public class PositionAndSizeAnimation extends Animation implements LayoutHandlingAnimation {
    ...
    public void applyTransformation(float f, Transformation transformation) {
        ...
        this.mView.layout(Math.round(f2), Math.round(f3), Math.round(f2 + (((float) this.mDeltaWidth) * f) + ((float) this.mStartWidth)), Math.round(f3 + (((float) this.mDeltaHeight) * f) + ((float) this.mStartHeight)));
    }
