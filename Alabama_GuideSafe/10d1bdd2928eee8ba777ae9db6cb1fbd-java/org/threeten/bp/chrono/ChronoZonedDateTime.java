...
public abstract class ChronoZonedDateTime<D extends ChronoLocalDate> extends DefaultInterfaceTemporal implements Temporal, Comparable<ChronoZonedDateTime<?>> {
    ...
    public int get(TemporalField temporalField) {
        ...
        if (ordinal == 28) {
            throw new UnsupportedTemporalTypeException(GeneratedOutlineSupport.outline12("Field too large for an int: ", temporalField));
        } else if (ordinal != 29) {
    ...
    public int compareTo(ChronoZonedDateTime<?> chronoZonedDateTime) {
        int compareLongs = MoreExecutors.compareLongs(toEpochSecond(), chronoZonedDateTime.toEpochSecond());
        if (compareLongs != 0) {
            return compareLongs;
        }
