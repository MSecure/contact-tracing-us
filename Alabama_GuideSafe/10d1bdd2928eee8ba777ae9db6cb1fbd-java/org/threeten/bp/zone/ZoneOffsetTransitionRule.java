...
public final class ZoneOffsetTransitionRule implements Serializable {
    ...
    public String toString() {
        StringBuilder outline17 = GeneratedOutlineSupport.outline17("TransitionRule[");
        ZoneOffset zoneOffset = this.offsetBefore;
        ZoneOffset zoneOffset2 = this.offsetAfter;
        if (zoneOffset != null) {
            outline17.append(zoneOffset2.totalSeconds - zoneOffset.totalSeconds > 0 ? "Gap " : "Overlap ");
            ...
            if (dayOfWeek != null) {
                byte b = this.dom;
                if (b == -1) {
                    outline17.append(dayOfWeek.name());
                    outline17.append(" on or before last day of ");
                    outline17.append(this.month.name());
                } else if (b < 0) {
                    outline17.append(dayOfWeek.name());
                    outline17.append(" on or before last day minus ");
                    outline17.append((-this.dom) - 1);
                    outline17.append(" of ");
                    outline17.append(this.month.name());
                } else {
                    outline17.append(dayOfWeek.name());
                    outline17.append(" on or after ");
                    outline17.append(this.month.name());
                    outline17.append(' ');
                    outline17.append((int) this.dom);
                }
            } else {
                outline17.append(this.month.name());
                outline17.append(' ');
            ...
            if (this.adjustDays == 0) {
                ...
                long floorDiv = MoreExecutors.floorDiv(secondOfDay, 60);
                if (floorDiv < 10) {
                    outline17.append(0);
                }
                outline17.append(floorDiv);
                outline17.append(':');
                long floorMod = (long) MoreExecutors.floorMod(secondOfDay, 60);
                if (floorMod < 10) {
                    outline17.append(0);
                }
                outline17.append(floorMod);
            }
            outline17.append(" ");
            outline17.append(this.timeDefinition);
            outline17.append(", standard offset ");
            outline17.append(this.standardOffset);
            outline17.append(']');
