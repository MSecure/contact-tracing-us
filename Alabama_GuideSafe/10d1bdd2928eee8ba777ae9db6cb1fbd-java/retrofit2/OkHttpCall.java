    ...
    public final Call createRawCall() throws IOException {
        ...
        if (length == parameterHandlerArr.length) {
            ...
            if (builder != null) {
                ...
                if (httpUrl == null) {
                    StringBuilder outline17 = GeneratedOutlineSupport.outline17("Malformed URL. Base: ");
                    outline17.append(requestBuilder.baseUrl);
                    outline17.append(", Relative: ");
                    outline17.append(requestBuilder.relativeUrl);
                    throw new IllegalArgumentException(outline17.toString());
                }
            }
            RequestBuilder.ContentTypeOverridingRequestBody contentTypeOverridingRequestBody = requestBuilder.body;
            ...
            if (mediaType != null) {
                if (contentTypeOverridingRequestBody != null) {
                    ...
                    requestBuilder.headersBuilder.add("Content-Type", mediaType.toString());
                }
            }
            Call newCall = factory.newCall(requestBuilder.requestBuilder.url(httpUrl).headers(requestBuilder.headersBuilder.build()).method(requestBuilder.method, contentTypeOverridingRequestBody).tag(Invocation.class, new Invocation(requestFactory2.method, arrayList)).build());
            if (newCall != null) {
                return newCall;
            }
            throw new NullPointerException("Call.Factory returned null.");
        }
        throw new IllegalArgumentException(GeneratedOutlineSupport.outline13(GeneratedOutlineSupport.outline18("Argument count (", length, ") doesn't match expected count ("), parameterHandlerArr.length, ")"));
    }
