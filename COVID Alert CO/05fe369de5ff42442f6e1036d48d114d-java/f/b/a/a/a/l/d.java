package f.b.a.a.a.l;

import android.content.Context;
import android.widget.Button;
import android.widget.TextView;
import com.google.android.apps.exposurenotification.exposure.ExposureHomeViewModel;
import e.p.t;
import f.b.a.a.a.i.b0;
import f.b.a.a.a.i.e0.a;
import f.b.a.a.a.j.w;
import gov.co.cdphe.exposurenotifications.R;
import l.b.a.e;
import l.b.a.f;
import l.b.a.g;
import l.b.a.h;
import l.b.a.r;

public final /* synthetic */ class d implements t {
    public final /* synthetic */ o a;

    public /* synthetic */ d(o oVar) {
        this.a = oVar;
    }

    @Override // e.p.t
    public final void a(Object obj) {
        int i2;
        int i3;
        o oVar = this.a;
        f.b.a.a.a.x.d dVar = (f.b.a.a.a.x.d) obj;
        boolean booleanValue = Boolean.valueOf(oVar.b0.c.f()).booleanValue();
        w wVar = oVar.a0;
        TextView textView = wVar.f2231d;
        TextView textView2 = wVar.f2233f;
        Button button = wVar.f2234g;
        ExposureHomeViewModel exposureHomeViewModel = oVar.b0;
        Context p0 = oVar.p0();
        e c = exposureHomeViewModel.f332f.c();
        a aVar = b0.a;
        long j2 = l.b.a.d.c(g.F(f.R(dVar.a()), h.f4300h).w(r.f4321g), c).b;
        long j3 = 1;
        if (j2 < 0) {
            b0.a.c("Negative time since exposure!");
        } else {
            j3 = Math.max(1L, j2 / 86400);
        }
        textView.setText(p0.getResources().getQuantityString(R.plurals.days_from_start_of_exposure, (int) j3, Long.valueOf(j3)));
        if (booleanValue) {
            i2 = R.string.exposure_details_url_revoked;
            button.setText(R.string.exposure_details_url_revoked);
            i3 = R.string.exposure_details_text_revoked;
        } else {
            int b = dVar.b();
            if (b == 1) {
                i2 = R.string.exposure_details_url_1;
                button.setText(R.string.exposure_details_url_1);
                i3 = R.string.exposure_details_text_1;
            } else if (b == 2) {
                i2 = R.string.exposure_details_url_2;
                button.setText(R.string.exposure_details_url_2);
                i3 = R.string.exposure_details_text_2;
            } else if (b == 3) {
                i2 = R.string.exposure_details_url_3;
                button.setText(R.string.exposure_details_url_3);
                i3 = R.string.exposure_details_text_3;
            } else if (b == 4) {
                i2 = R.string.exposure_details_url_4;
                button.setText(R.string.exposure_details_url_4);
                i3 = R.string.exposure_details_text_4;
            } else {
                return;
            }
        }
        textView2.setText(i3);
        oVar.C0(oVar.A(i2));
    }
}
