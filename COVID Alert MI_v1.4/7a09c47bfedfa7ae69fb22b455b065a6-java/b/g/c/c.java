package b.g.c;

import android.content.Context;
import android.content.res.TypedArray;
import android.content.res.XmlResourceParser;
import android.util.SparseArray;
import android.util.Xml;
import android.view.LayoutInflater;
import android.view.ViewGroup;
import androidx.constraintlayout.widget.ConstraintLayout;
import b.g.c.d;
import java.io.IOException;
import java.util.ArrayList;
import org.xmlpull.v1.XmlPullParser;
import org.xmlpull.v1.XmlPullParserException;

public class c {

    /* renamed from: a  reason: collision with root package name */
    public SparseArray<a> f1588a = new SparseArray<>();

    /* renamed from: b  reason: collision with root package name */
    public SparseArray<d> f1589b = new SparseArray<>();

    public static class a {

        /* renamed from: a  reason: collision with root package name */
        public int f1590a;

        /* renamed from: b  reason: collision with root package name */
        public ArrayList<b> f1591b = new ArrayList<>();

        /* renamed from: c  reason: collision with root package name */
        public int f1592c = -1;

        /* renamed from: d  reason: collision with root package name */
        public d f1593d;

        public a(Context context, XmlPullParser xmlPullParser) {
            TypedArray obtainStyledAttributes = context.obtainStyledAttributes(Xml.asAttributeSet(xmlPullParser), i.State);
            int indexCount = obtainStyledAttributes.getIndexCount();
            for (int i = 0; i < indexCount; i++) {
                int index = obtainStyledAttributes.getIndex(i);
                if (index == i.State_android_id) {
                    this.f1590a = obtainStyledAttributes.getResourceId(index, this.f1590a);
                } else if (index == i.State_constraints) {
                    this.f1592c = obtainStyledAttributes.getResourceId(index, this.f1592c);
                    String resourceTypeName = context.getResources().getResourceTypeName(this.f1592c);
                    context.getResources().getResourceName(this.f1592c);
                    if ("layout".equals(resourceTypeName)) {
                        d dVar = new d();
                        this.f1593d = dVar;
                        dVar.c((ConstraintLayout) LayoutInflater.from(context).inflate(this.f1592c, (ViewGroup) null));
                    }
                }
            }
            obtainStyledAttributes.recycle();
        }
    }

    public static class b {

        /* renamed from: a  reason: collision with root package name */
        public float f1594a = Float.NaN;

        /* renamed from: b  reason: collision with root package name */
        public float f1595b = Float.NaN;

        /* renamed from: c  reason: collision with root package name */
        public float f1596c = Float.NaN;

        /* renamed from: d  reason: collision with root package name */
        public float f1597d = Float.NaN;

        /* renamed from: e  reason: collision with root package name */
        public int f1598e = -1;

        /* renamed from: f  reason: collision with root package name */
        public d f1599f;

        public b(Context context, XmlPullParser xmlPullParser) {
            TypedArray obtainStyledAttributes = context.obtainStyledAttributes(Xml.asAttributeSet(xmlPullParser), i.Variant);
            int indexCount = obtainStyledAttributes.getIndexCount();
            for (int i = 0; i < indexCount; i++) {
                int index = obtainStyledAttributes.getIndex(i);
                if (index == i.Variant_constraints) {
                    this.f1598e = obtainStyledAttributes.getResourceId(index, this.f1598e);
                    String resourceTypeName = context.getResources().getResourceTypeName(this.f1598e);
                    context.getResources().getResourceName(this.f1598e);
                    if ("layout".equals(resourceTypeName)) {
                        d dVar = new d();
                        this.f1599f = dVar;
                        dVar.c((ConstraintLayout) LayoutInflater.from(context).inflate(this.f1598e, (ViewGroup) null));
                    }
                } else if (index == i.Variant_region_heightLessThan) {
                    this.f1597d = obtainStyledAttributes.getDimension(index, this.f1597d);
                } else if (index == i.Variant_region_heightMoreThan) {
                    this.f1595b = obtainStyledAttributes.getDimension(index, this.f1595b);
                } else if (index == i.Variant_region_widthLessThan) {
                    this.f1596c = obtainStyledAttributes.getDimension(index, this.f1596c);
                } else if (index == i.Variant_region_widthMoreThan) {
                    this.f1594a = obtainStyledAttributes.getDimension(index, this.f1594a);
                }
            }
            obtainStyledAttributes.recycle();
        }
    }

    public c(Context context, ConstraintLayout constraintLayout, int i) {
        XmlResourceParser xml = context.getResources().getXml(i);
        a aVar = null;
        try {
            for (int eventType = xml.getEventType(); eventType != 1; eventType = xml.next()) {
                if (eventType == 0) {
                    xml.getName();
                } else if (eventType == 2) {
                    String name = xml.getName();
                    char c2 = 65535;
                    switch (name.hashCode()) {
                        case -1349929691:
                            if (name.equals("ConstraintSet")) {
                                c2 = 4;
                                break;
                            }
                            break;
                        case 80204913:
                            if (name.equals("State")) {
                                c2 = 2;
                                break;
                            }
                            break;
                        case 1382829617:
                            if (name.equals("StateSet")) {
                                c2 = 1;
                                break;
                            }
                            break;
                        case 1657696882:
                            if (name.equals("layoutDescription")) {
                                c2 = 0;
                                break;
                            }
                            break;
                        case 1901439077:
                            if (name.equals("Variant")) {
                                c2 = 3;
                                break;
                            }
                            break;
                    }
                    if (!(c2 == 0 || c2 == 1)) {
                        if (c2 == 2) {
                            aVar = new a(context, xml);
                            this.f1588a.put(aVar.f1590a, aVar);
                        } else if (c2 == 3) {
                            b bVar = new b(context, xml);
                            if (aVar != null) {
                                aVar.f1591b.add(bVar);
                            }
                        } else if (c2 == 4) {
                            a(context, xml);
                        }
                    }
                }
            }
        } catch (XmlPullParserException e2) {
            e2.printStackTrace();
        } catch (IOException e3) {
            e3.printStackTrace();
        }
    }

    /* JADX INFO: Can't fix incorrect switch cases order, some code will duplicate */
    /* JADX WARNING: Code restructure failed: missing block: B:104:0x01ca, code lost:
        continue;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:43:0x00c8, code lost:
        if (r1.equals("PropertySet") != false) goto L_0x00e0;
     */
    public final void a(Context context, XmlPullParser xmlPullParser) {
        d.a aVar;
        d dVar = new d();
        int attributeCount = xmlPullParser.getAttributeCount();
        for (int i = 0; i < attributeCount; i++) {
            if ("id".equals(xmlPullParser.getAttributeName(i))) {
                String attributeValue = xmlPullParser.getAttributeValue(i);
                int identifier = attributeValue.contains("/") ? context.getResources().getIdentifier(attributeValue.substring(attributeValue.indexOf(47) + 1), "id", context.getPackageName()) : -1;
                if (identifier == -1 && attributeValue.length() > 1) {
                    identifier = Integer.parseInt(attributeValue.substring(1));
                }
                try {
                    int eventType = xmlPullParser.getEventType();
                    d.a aVar2 = null;
                    while (eventType != 1) {
                        if (eventType != 0) {
                            char c2 = 3;
                            if (eventType == 2) {
                                String name = xmlPullParser.getName();
                                switch (name.hashCode()) {
                                    case -2025855158:
                                        if (name.equals("Layout")) {
                                            c2 = 5;
                                            break;
                                        }
                                        c2 = 65535;
                                        break;
                                    case -1984451626:
                                        if (name.equals("Motion")) {
                                            c2 = 6;
                                            break;
                                        }
                                        c2 = 65535;
                                        break;
                                    case -1269513683:
                                        break;
                                    case -1238332596:
                                        if (name.equals("Transform")) {
                                            c2 = 4;
                                            break;
                                        }
                                        c2 = 65535;
                                        break;
                                    case -71750448:
                                        if (name.equals("Guideline")) {
                                            c2 = 1;
                                            break;
                                        }
                                        c2 = 65535;
                                        break;
                                    case 1331510167:
                                        if (name.equals("Barrier")) {
                                            c2 = 2;
                                            break;
                                        }
                                        c2 = 65535;
                                        break;
                                    case 1791837707:
                                        if (name.equals("CustomAttribute")) {
                                            c2 = 7;
                                            break;
                                        }
                                        c2 = 65535;
                                        break;
                                    case 1803088381:
                                        if (name.equals("Constraint")) {
                                            c2 = 0;
                                            break;
                                        }
                                        c2 = 65535;
                                        break;
                                    default:
                                        c2 = 65535;
                                        break;
                                }
                                switch (c2) {
                                    case 0:
                                        aVar = dVar.e(context, Xml.asAttributeSet(xmlPullParser));
                                        aVar2 = aVar;
                                        break;
                                    case 1:
                                        aVar = dVar.e(context, Xml.asAttributeSet(xmlPullParser));
                                        aVar.f1608d.f1611a = true;
                                        aVar.f1608d.f1612b = true;
                                        aVar2 = aVar;
                                        break;
                                    case 2:
                                        aVar = dVar.e(context, Xml.asAttributeSet(xmlPullParser));
                                        aVar.f1608d.d0 = 1;
                                        aVar2 = aVar;
                                        break;
                                    case 3:
                                        if (aVar2 != null) {
                                            aVar2.f1606b.a(context, Xml.asAttributeSet(xmlPullParser));
                                            break;
                                        } else {
                                            throw new RuntimeException("XML parser error must be within a Constraint " + xmlPullParser.getLineNumber());
                                        }
                                    case 4:
                                        if (aVar2 != null) {
                                            aVar2.f1609e.a(context, Xml.asAttributeSet(xmlPullParser));
                                            break;
                                        } else {
                                            throw new RuntimeException("XML parser error must be within a Constraint " + xmlPullParser.getLineNumber());
                                        }
                                    case 5:
                                        if (aVar2 != null) {
                                            aVar2.f1608d.a(context, Xml.asAttributeSet(xmlPullParser));
                                            break;
                                        } else {
                                            throw new RuntimeException("XML parser error must be within a Constraint " + xmlPullParser.getLineNumber());
                                        }
                                    case 6:
                                        if (aVar2 != null) {
                                            aVar2.f1607c.a(context, Xml.asAttributeSet(xmlPullParser));
                                            break;
                                        } else {
                                            throw new RuntimeException("XML parser error must be within a Constraint " + xmlPullParser.getLineNumber());
                                        }
                                    case 7:
                                        if (aVar2 != null) {
                                            a.a(context, xmlPullParser, aVar2.f1610f);
                                            break;
                                        } else {
                                            throw new RuntimeException("XML parser error must be within a Constraint " + xmlPullParser.getLineNumber());
                                        }
                                }
                            } else if (eventType != 3) {
                                continue;
                            } else {
                                String name2 = xmlPullParser.getName();
                                if ("ConstraintSet".equals(name2)) {
                                    this.f1589b.put(identifier, dVar);
                                    return;
                                } else if (name2.equalsIgnoreCase("Constraint")) {
                                    dVar.f1604c.put(Integer.valueOf(aVar2.f1605a), aVar2);
                                    aVar2 = null;
                                }
                            }
                        } else {
                            xmlPullParser.getName();
                        }
                        eventType = xmlPullParser.next();
                    }
                } catch (XmlPullParserException e2) {
                    e2.printStackTrace();
                } catch (IOException e3) {
                    e3.printStackTrace();
                }
                this.f1589b.put(identifier, dVar);
                return;
            }
        }
    }
}
