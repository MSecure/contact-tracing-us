...
public class ForceStopRunnable implements Runnable {
        ...
        try {
            ...
            if (longValue != null && longValue.longValue() == 1) {
                ...
                if (preferenceUtils != null) {
                    ((PreferenceDao_Impl) preferenceUtils.mWorkDatabase.preferenceDao()).insertPreference(new Preference("reschedule_needed", false));
                } else {
                    throw null;
                }
            } else {
    ...
    public void run() {
        ...
        if (workManagerImpl.mRemoteWorkManager == null) {
            ...
            Logger.get().debug(TAG, String.format("Is default app process = %s", Boolean.valueOf(z)), new Throwable[0]);
        }
        if (z) {
            while (true) {
