...
public final class OsSharedRealm implements Closeable, NativeObject {
    ...
    public static class VersionID implements Comparable<VersionID> {
        ...
        public String toString() {
            StringBuilder outline15 = GeneratedOutlineSupport.outline15("VersionID{version=");
            outline15.append(this.version);
            outline15.append(", index=");
            outline15.append(this.index);
            outline15.append('}');
        ...
        if (temporaryDirectory == null) {
            ...
            if (file.isDirectory() || file.mkdirs() || file.isDirectory()) {
                if (!absolutePath.endsWith(ColorPropConverter.PATH_DELIMITER)) {
                    absolutePath = GeneratedOutlineSupport.outline8(absolutePath, ColorPropConverter.PATH_DELIMITER);
                }
                nativeInit(absolutePath);
                temporaryDirectory = file;
                return;
            }
            throw new IOException(GeneratedOutlineSupport.outline8("failed to create temporary directory: ", absolutePath));
        }
    }
