...
public class CollectionsKt__CollectionsKt {
    ...
    public static final <T> Set<T> toSet(Iterable<? extends T> iterable) {
        ...
        if (iterable instanceof Collection) {
            ...
            if (size != 1) {
                LinkedHashSet linkedHashSet = new LinkedHashSet(PathParser.mapCapacity(collection.size()));
                toCollection(iterable, linkedHashSet);
                return linkedHashSet;
            }
            return PathParser.setOf(iterable instanceof List ? ((List) iterable).get(0) : iterable.iterator().next());
        }
        ...
        return PathParser.setOf(linkedHashSet2.iterator().next());
    }
}
