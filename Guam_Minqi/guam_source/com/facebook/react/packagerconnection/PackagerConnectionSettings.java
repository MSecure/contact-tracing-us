...
public class PackagerConnectionSettings {
    ...
    public String getDebugServerHost() {
        ...
        if (serverIpAddress.equals("localhost")) {
            StringBuilder outline15 = GeneratedOutlineSupport.outline15("You seem to be running on device. Run '");
            Integer valueOf = Integer.valueOf(this.mAppContext.getResources().getInteger(R$integer.react_native_dev_server_port));
            outline15.append("adb reverse tcp:" + valueOf + " tcp:" + valueOf);
            outline15.append("' to forward the debug server's port to the device.");
            FLog.w("PackagerConnectionSettings", outline15.toString());
        }
