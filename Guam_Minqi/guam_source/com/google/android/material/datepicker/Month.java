...
public final class Month implements Comparable<Month>, Parcelable {
    ...
    public Month(Calendar calendar) {
        calendar.set(5, 1);
        Calendar dayCopy = ReactYogaConfigProvider.getDayCopy(calendar);
        this.firstOfMonth = dayCopy;
        this.month = dayCopy.get(2);
        SimpleDateFormat simpleDateFormat = new SimpleDateFormat("MMMM, yyyy", Locale.getDefault());
        simpleDateFormat.setTimeZone(ReactYogaConfigProvider.getTimeZone());
    ...
    public static Month create(long j) {
        Calendar utcCalendar = ReactYogaConfigProvider.getUtcCalendar();
        utcCalendar.setTimeInMillis(j);
        return new Month(utcCalendar);
    }

    public static Month today() {
        return new Month(ReactYogaConfigProvider.getTodayCalendar());
    }
    ...
    public Month monthsLater(int i) {
        Calendar dayCopy = ReactYogaConfigProvider.getDayCopy(this.firstOfMonth);
        dayCopy.add(2, i);
