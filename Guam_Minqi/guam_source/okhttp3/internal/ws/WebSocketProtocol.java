...
public final class WebSocketProtocol {
    ...
    public static String closeCodeExceptionMessage(int i) {
        if (i < 1000 || i >= 5000) {
            return GeneratedOutlineSupport.outline4("Code must be in range [1000,5000): ", i);
        }
        if ((i < 1004 || i > 1006) && (i < 1012 || i > 2999)) {
            return null;
        }
        return GeneratedOutlineSupport.outline5("Code ", i, " is reserved and may not be used.");
    }
    ...
    public static void toggleMask(Buffer.UnsafeCursor unsafeCursor, byte[] bArr) {
        ...
        do {
            ...
            if (j == unsafeCursor.buffer.size) {
                ...
                i = unsafeCursor.seek(0);
            } else {
                i = unsafeCursor.seek(j + ((long) (unsafeCursor.end - unsafeCursor.start)));
            }
        } while (i != -1);
    }
