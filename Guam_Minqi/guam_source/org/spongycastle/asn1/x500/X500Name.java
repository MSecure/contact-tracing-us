            ...
            for (int i2 = 0; i2 != rDNs.length; i2++) {
                if (rDNs[i2].isMultiValued()) {
                    ...
                    for (int i3 = 0; i3 != typesAndValues.length; i3++) {
                        i = (i ^ typesAndValues[i3].type.hashCode()) ^ PathParser.canonicalize(PathParser.valueToString(typesAndValues[i3].value)).hashCode();
                    }
                } else {
                    i = (i ^ rDNs[i2].getFirst().type.hashCode()) ^ PathParser.canonicalize(PathParser.valueToString(rDNs[i2].getFirst().value)).hashCode();
                }
            }
            ...
            for (RDN rdn : rDNs) {
                ...
                if (rdn.isMultiValued()) {
                    ...
                    for (int i = 0; i != typesAndValues.length; i++) {
                        ...
                        PathParser.appendTypeAndValue(stringBuffer, typesAndValues[i], hashtable);
                    }
                } else if (rdn.getFirst() != null) {
                    PathParser.appendTypeAndValue(stringBuffer, rdn.getFirst(), hashtable);
                }
            }
