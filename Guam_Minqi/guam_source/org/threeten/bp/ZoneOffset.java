...
public final class ZoneOffset extends ZoneId implements TemporalAccessor, TemporalAdjuster, Comparable<ZoneOffset>, Serializable {
    ...
    public static ZoneOffset of(String str) {
        ...
        int i3;
        char charAt;
        PathParser.requireNonNull(str, "offsetId");
        ...
        if (length == 2) {
            ...
            if (length == 5) {
                ...
                if (length == 7) {
                    ...
                    throw new DateTimeException(GeneratedOutlineSupport.outline8("Invalid ID for ZoneOffset, invalid format: ", str));
                }
                charAt = str.charAt(0);
                if (charAt == '+' && charAt != '-') {
                    throw new DateTimeException(GeneratedOutlineSupport.outline8("Invalid ID for ZoneOffset, plus/minus not found when expected: ", str));
                } else if (charAt == '-') {
    ...
    public static ZoneOffset ofHoursMinutesSeconds(int i, int i2, int i3) {
        if (i < -18 || i > 18) {
            throw new DateTimeException(GeneratedOutlineSupport.outline5("Zone offset hours not in valid range: value ", i, " is not in the range -18 to 18"));
        }
        if (i > 0) {
            if (i2 < 0 || i3 < 0) {
        ...
        if (Math.abs(i2) > 59) {
            StringBuilder outline15 = GeneratedOutlineSupport.outline15("Zone offset minutes not in valid range: abs(value) ");
            outline15.append(Math.abs(i2));
            outline15.append(" is not in the range 0 to 59");
            throw new DateTimeException(outline15.toString());
        } else if (Math.abs(i3) > 59) {
            StringBuilder outline152 = GeneratedOutlineSupport.outline15("Zone offset seconds not in valid range: abs(value) ");
            outline152.append(Math.abs(i3));
            outline152.append(" is not in the range 0 to 59");
    ...
    public long getLong(TemporalField temporalField) {
        ...
        throw new DateTimeException(GeneratedOutlineSupport.outline10("Unsupported field: ", temporalField));
    }

    @Override // org.threeten.bp.ZoneId
    public ZoneRules getRules() {
        PathParser.requireNonNull(this, "offset");
