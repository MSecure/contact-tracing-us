...
public class i extends j {
    ...
    public Typeface a(Context context, c cVar, Resources resources, int i) {
        ...
        while (true) {
            ...
            d dVar = dVarArr[i3];
            try {
                Font.Builder weight = new Font.Builder(resources, dVar.f1731f).setWeight(dVar.f1727b);
                if (!dVar.f1728c) {
                    i4 = 0;
                }
                Font build = weight.setSlant(i4).setTtcIndex(dVar.f1730e).setFontVariationSettings(dVar.f1729d).build();
                if (builder == null) {
    ...
    public Typeface b(Context context, CancellationSignal cancellationSignal, b.f[] fVarArr, int i) {
        ...
        while (true) {
            ...
            b.f fVar = fVarArr[i3];
            try {
                ParcelFileDescriptor openFileDescriptor = contentResolver.openFileDescriptor(fVar.f1791a, "r", cancellationSignal);
                if (openFileDescriptor != null) {
                    try {
                        Font.Builder weight = new Font.Builder(openFileDescriptor).setWeight(fVar.f1793c);
                        if (!fVar.f1794d) {
                            i4 = 0;
                        }
                        Font build = weight.setSlant(i4).setTtcIndex(fVar.f1792b).build();
                        if (builder == null) {
                            builder = new FontFamily.Builder(build);
                        } else {
                            builder.addFont(build);
