...
public class OtpView extends k {
    ...
    private void drawCursor(Canvas canvas) {
        if (this.drawCursor) {
            PointF pointF = this.itemCenterPoint;
            float f = pointF.x;
            float f2 = pointF.y - (this.cursorHeight / 2.0f);
    ...
    private void drawOtpLine(Canvas canvas, int i) {
        ...
        if (getText() == null || !this.hideLineWhenFilled || i >= getText().length()) {
            if (this.otpViewItemSpacing != 0 || (i2 = this.otpViewItemCount) <= 1) {
                ...
                rectF.set(rectF2.left - f, f2 - f, rectF2.right + f, f2 + f);
            ...
            this.paint.setStyle(Paint.Style.FILL);
            this.paint.setStrokeWidth(((float) this.lineWidth) / 10.0f);
            float f3 = ((float) this.lineWidth) / 2.0f;
            RectF rectF4 = this.itemLineRect;
            RectF rectF22 = this.itemBorderRect;
            float f22 = rectF22.bottom;
            rectF4.set(rectF22.left - f3, f22 - f3, rectF22.right + f3, f22 + f3);
    ...
    private void drawTextAtBox(Canvas canvas, Paint paint2, CharSequence charSequence, int i) {
        ...
        float f = pointF.x;
        float f2 = pointF.y;
        float abs = f - (Math.abs((float) this.textRect.width()) / 2.0f);
        Rect rect = this.textRect;
        canvas.drawText(charSequence, i, i2, abs - ((float) rect.left), ((Math.abs((float) rect.height()) / 2.0f) + f2) - ((float) this.textRect.bottom), paint2);
    }
    ...
    private void setupAnimator() {
        ...
        this.defaultAddAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            /* class gov.nc.dhhs.exposurenotification.otpview.OtpView.AnonymousClass1 */
    ...
    private void updateCenterPoint() {
        ...
        RectF rectF2 = this.itemBorderRect;
        float f = rectF2.top;
        this.itemCenterPoint.set(abs, (Math.abs(rectF2.height()) / 2.0f) + f);
    }
    ...
    private void updateItemRectF(int i) {
        float f = ((float) this.lineWidth) / 2.0f;
        int s = m.s(this) + getScrollX();
        int i2 = this.otpViewItemSpacing;
        float f2 = ((float) (((this.otpViewItemWidth + i2) * i) + s)) + f;
        if (i2 == 0 && i > 0) {
            f2 -= (float) (this.lineWidth * i);
        }
        float f3 = (((float) this.otpViewItemWidth) + f2) - ((float) this.lineWidth);
        float paddingTop = ((float) (getPaddingTop() + getScrollY())) + f;
        this.itemBorderRect.set(f2, paddingTop, f3, (((float) this.otpViewItemHeight) + paddingTop) - ((float) this.lineWidth));
    }
    ...
    public void onMeasure(int i, int i2) {
        ...
        if (mode != 1073741824) {
            ...
            size = getPaddingStart() + m.r(this) + (i4 * this.otpViewItemWidth) + ((i4 - 1) * this.otpViewItemSpacing);
            if (this.otpViewItemSpacing == 0) {
