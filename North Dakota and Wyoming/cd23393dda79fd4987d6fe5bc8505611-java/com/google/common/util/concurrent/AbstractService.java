...
public abstract class AbstractService implements Service {
    ...
    public static final class StateSnapshot {
        ...
        StateSnapshot(Service.State state2, boolean z, @NullableDecl Throwable th) {
            ...
            Preconditions.checkArgument(!((state2 == Service.State.FAILED ? true : z2) ^ (th != null)), "A failure cause should be set if and only if the state is failed.  Got %s and %s instead.", state2, th);
            this.state = state2;
